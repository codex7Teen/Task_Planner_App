...
class TodoSearchBarField extends StatefulWidget {
  ...
  const TodoSearchBarField(
      {super.key, required this.onCancelTapped, required this.todoListx});
...
class _TodoSearchBarFieldState extends State<TodoSearchBarField> {
  ...
  void searchTodoFields(String query) {
    List<TodoModel> filteredTodos = widget.todoListx
        .where((todo) => todo.name.toLowerCase().contains(query.toLowerCase()))
        .toList();
